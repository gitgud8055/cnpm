...
mydb = mysql.connector.connect(
  host="cnpm-ittn.mysql.database.azure.com",
  user="tuecm",
  password="12345cmt#",
  database = "mydb"
)
...
def create(table_name, value):
    ...
    # chua commit data
    command = "insert into {} values ({})".format(table_name, ("%s," * len(col_info[table_name]))[:-1])
    if (len(value) == 0): 
        return 
...
def delete(table_name, conditions):
    ...
    command = "delete from {} {}".format(table_name, 
                "where " + ' and '.join([(x[1].format(x[0])) for x in conditions]) if conditions else "")
    #print(command)
    cursor.execute(command)
        ...
        for pos, y in enumerate(table_name):
            ...
            for name in col_info[y]:
                if name[0] in list_name:
                    ans[find(pos)] += f" join ({ans[find(id)]}) on {x}.{name[0]} = {y}.{name[0]} "
...
def show(table_name, column_name = None, conditions = None, group_by = None, special_column_name = None, condition_aggressive = None, sort_by = None, limit = None):
    """
    ...
    command = "select {} from ({}) {} {} {} {} {}"
    command = command.format( ','.join(full_column_name), 
                   join_all(table_name), 
                   "where " + ' and '.join([x[1].format(find(x[0])) for x in conditions]) if conditions else "", 
                   f"group by {find(group_by)}" if group_by else "",
                    "having " + ' and '.join([x[1].format(find(x[0])) for x in condition_aggressive]) if condition_aggressive else "",
...
for x in cursor:
    print(x)
show(["tai_khoan", "ho_gd"], [], [("id_tai_khoan","{} between 1 and 7"), ("id_ho","{} =1")], limit = 2, sort_by="id_tai_khoan")
